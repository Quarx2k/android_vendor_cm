...
ifdef CM_NIGHTLY
    CM_VERSION :=$(PRODUCT_VERSION_MAJOR)-$(shell date +%m%d%Y)-NIGHTLY-$(CM_BUILD)
    MODVERSION :=$(PRODUCT_VERSION_MAJOR)-NIGHTLY-$(shell date +%y%m%d)-$(PRODUCT_RELEASE_NAME)
else
    ifdef CM_SNAPSHOT
        CM_VERSION := $(PRODUCT_VERSION_MAJOR)-$(shell date +%m%d%Y)-SNAPSHOT-$(CM_BUILD)$(CM_EXTRAVERSION)
        MODVERSION := $(PRODUCT_VERSION_MAJOR)-SNAPSHOT-$(shell date +%y%m%d)-$(CM_BUILD)$(CM_EXTRAVERSION)
    else
        ifdef CM_RELEASE
