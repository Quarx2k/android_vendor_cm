...
ifdef CM_NIGHTLY
    PRODUCT_PROPERTY_OVERRIDES += \
        ro.cm.version=$(PRODUCT_VERSION_MAJOR)-NIGHTLY-$(shell date +%y%m%d)-$(PRODUCT_RELEASE_NAME)
else
    ifdef CM_RELEASE
        PRODUCT_PROPERTY_OVERRIDES += \
            ro.cm.version=$(PRODUCT_VERSION_MAJOR)-$(shell date +%m%d%Y)-SNAPSHOT-$(PRODUCT_DEVICE)$(CM_EXTRAVERSION)
    else
        ifdef CM_RELEASE
            PRODUCT_PROPERTY_OVERRIDES += \
                ro.cm.version=$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(PRODUCT_DEVICE)
        else
            PRODUCT_PROPERTY_OVERRIDES += \
                ro.cm.version=$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(PRODUCT_DEVICE)-KANG
        endif
    endif
endif
